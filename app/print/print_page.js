...
function isImageUrl(url){return url&&/\.(jpg|jpeg|png|gif|webp)/i.test(url);}
    ...
    <div className="voucher-a5">
      <div className="vh-row">
        <div className="vh-logo">
          {logo?<img src={logo} alt="logo" className="vh-logo-img"/>:<div className="vh-logo-text">{voucher.companyName}</div>}
        </div>
      ...
      <div className="vi-grid">
        <div className="vi-cell"><span className="vi-label">Company / الشركة</span><span className="vi-val">{voucher.companyName}</span></div>
        <div className="vi-cell"><span className="vi-label">Department / القسم</span><span className="vi-val">{voucher.departmentName}</span></div>
        <div className="vi-cell"><span className="vi-label">Prepared By / أعده</span><span className="vi-val">{voucher.staffName}</span></div>
        <div className="vi-cell"><span className="vi-label">Receiver / المستلم</span><span className="vi-val vi-bold">{voucher.receiverName}</span></div>
      </div>
        ...
        <tbody>
          {rows.map((item,i)=>(
            <tr key={i} className={i%2===0?"vt-r":"vt-r vt-alt"}>
              ...
              <td className="vt-d" style={{textAlign:"right",fontWeight:item.amount?600:400}}>{item.amount?fmtAED(item.amount):""}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div className="vb-total">
        <div className="vb-words"><span className="vi-label">Amount in Words / المبلغ كتابةً:&nbsp;</span><span className="vb-words-text">{numberToWords(voucher.totalAmount)}</span></div>
        <div className="vb-amount-box"><span className="vb-amount-label">Total / المجموع</span><span className="vb-amount">AED {fmtAED(voucher.totalAmount)}</span></div>
      </div>
      <div className="vs-row">
        {[["General Manager","المدير العام"],["Department Head","رئيس القسم"],["Finance","المالية"],["Cashier","أمين الصندوق"]].map(([en,ar])=>(
          <div key={en} className="vs-cell"><div className="vs-line"/><div className="vs-en">{en}</div><div className="vs-ar">{ar}</div></div>
        ))}
      </div>
      <div className="vf-row">
        <span className="vf-stamp" style={{color:voucher.status==="paid"?"#2d6a4f":"#856404",borderColor:voucher.status==="paid"?"#2d6a4f":"#ffc107"}}>
    ...
    <div className="att-page">
      ...
      <div className="att-body">
        {isImageUrl(attachment.originalUrl)
          ...
          :<iframe src={attachment.originalUrl} title={attachment.fileName} className="att-iframe"/>
        }
      </div>
    </div>
  );
...
export default function PrintPage(){
  ...
  // Pre-select voucher from URL param (when coming from View popup)
  useEffect(()=>{
    if(typeof window!=="undefined"&&vouchers.length>0){
      const ref=new URLSearchParams(window.location.search).get("ref");
      if(ref){
        const match=vouchers.find(v=>v.refNumber===ref);
        if(match){ setSelectedIds([match.id]); setExpandedIds(new Set([match.id])); }
      }
    }
  },[vouchers]);
  ...
  async function checkAuth(){
    ...
    if(!user||!["cashier","finance","admin"].includes(user.role)){router.push("/login");return;}
    setCurrentUser(user); await loadData(user);
  }
  ...
  async function loadData(user){
    ...
    if(user.role==="cashier"&&user.companies?.length) allV=allV.filter(v=>user.companies.includes(v.companyId));
    setVouchers(allV.sort((a,b)=>(b.createdAt?.seconds||0)-(a.createdAt?.seconds||0)));
  }

  function showBanner(text,type="success"){setMsgBanner({text,type});setTimeout(()=>setMsgBanner({text:"",type:"success"}),4000);}
  ...
  async function handleLogoUpload(e,companyId){
    ...
    const fd=new FormData(); fd.append("file",file); fd.append("upload_preset","expense_voucher");
    try{
      ...
      if(data.secure_url){
        ...
        setCompanies(prev=>prev.map(c=>c.id===companyId?{...c,logoUrl:data.secure_url}:c));
  ...
  function selectAll(){setSelectedIds(filteredVouchers.map(v=>v.id));setExpandedIds(new Set(filteredVouchers.map(v=>v.id)));}
  function selectUnprinted(){const ids=filteredVouchers.filter(v=>!v.printed).map(v=>v.id);setSelectedIds(ids);setExpandedIds(new Set(ids));}
  function clearSelection(){setSelectedIds([]);setExpandedIds(new Set());}

  function toggleExpand(id,e){e.stopPropagation();setExpandedIds(prev=>{const next=new Set(prev);next.has(id)?next.delete(id):next.add(id);return next;});}

  function toggleSkipAtt(voucherId,url){
    setSkippedAtts(prev=>{const cur=new Set(prev[voucherId]||[]);cur.has(url)?cur.delete(url):cur.add(url);return{...prev,[voucherId]:cur};});
  }
  function isAttSkipped(voucherId,url){return skippedAtts[voucherId]?.has(url)||false;}
  function activeAtts(voucher){return(voucher.attachments||[]).filter(a=>!isAttSkipped(voucher.id,a.originalUrl));}

  async function markPrinted(ids){
    for(const id of ids){
      await updateDoc(doc(db,"vouchers",id),{printed:true,printedAt:new Date().toISOString(),printedBy:currentUser?.name||""});
    }
  ...
  const filteredVouchers=vouchers.filter(v=>{
    ...
    const mq=!searchTerm||v.refNumber?.toLowerCase().includes(q)||v.receiverName?.toLowerCase().includes(q)||v.companyName?.toLowerCase().includes(q);
    return ms&&mc&&mq;
  });

  const toPrint=vouchers.filter(v=>selectedIds.includes(v.id));
  const unprinted=filteredVouchers.filter(v=>!v.printed).length;
  function getLogo(companyId){return companies.find(c=>c.id===companyId)?.logoUrl||null;}

  // Smart print plan
  function buildPrintPlan(){
    const plan=[]; let noAttBuffer=null;
    for(const voucher of toPrint){
      const atts=activeAtts(voucher);const logo=getLogo(voucher.companyId);
      if(atts.length===0){
        if(noAttBuffer){plan.push({type:"pair",v1:noAttBuffer,v2:{voucher,logo}});noAttBuffer=null;}
        else{noAttBuffer={voucher,logo};}
      }else{
        if(noAttBuffer){plan.push({type:"solo",v1:noAttBuffer});noAttBuffer=null;}
    ...
    <>
      ...
      <div className="screen-only">
        <div style={S.header}>
          <div>
            ...
            <div style={S.headerSub}>2 per page (no attachments) · Smart fit with attachments</div>
          </div>
        ...
        <div style={S.content}>
              ...
              <div style={S.logoGrid}>
                {companies.map(c=>(
                  <div key={c.id} style={S.logoCard}>
                    ...
                    {c.logoUrl?<img src={c.logoUrl} alt="logo" style={S.logoPreview}/>:<div style={S.logoEmpty}>No logo</div>}
                    <label style={S.uploadBtn}>
                      {c.logoUrl?"🔄 Replace":"📁 Upload Logo"}
                      <input type="file" accept="image/*" style={{display:"none"}} onChange={e=>handleLogoUpload(e,c.id)}/>
                    </label>
                  </div>
                ))}
              </div>
          ...
          <div style={S.section}>
            <div style={S.filterRow}>
              <input style={S.searchInput} placeholder="🔍  Search ref, receiver, company..."
          ...
          <div style={S.printControls}>
            <div style={S.ctrlLeft}>
              <button style={S.ctrlBtn} onClick={selectAll}>☑ All ({filteredVouchers.length})</button>
              <button style={S.ctrlBtn} onClick={selectUnprinted}>☐ Unprinted ({unprinted})</button>
              <button style={S.ctrlBtn} onClick={clearSelection}>✕ Clear</button>
            </div>
            <div style={S.ctrlRight}>
              {selectedIds.length>0&&<span style={S.selCount}>{selectedIds.length} selected</span>}
              <button style={selectedIds.length?S.printBtn:S.printBtnOff} onClick={handlePrint} disabled={!selectedIds.length}>
                🖨️ Print
              </button>
            </div>
          </div>
          ...
          {selectedIds.length>0&&(
            <div style={S.layoutHint}>
              <span style={{fontSize:12,color:"#555"}}>📐 Layout: </span>
              {printPlan.map((unit,i)=>{
                if(unit.type==="pair") return <span key={i} style={S.hintChip}>📄+📄</span>;
                if(unit.type==="solo") return <span key={i} style={S.hintChip}>📄</span>;
                return <span key={i} style={{...S.hintChip,backgroundColor:"#e8f4fd",color:"#1a6fa8"}}>📄+📎×{unit.atts.length}</span>;
              })}
            </div>
          )}
          ...
          {filteredVouchers.map(v=>{
            ...
            const sc={paid:{bg:"#cce5ff",fg:"#004085"},pending:{bg:"#fff3cd",fg:"#856404"},rejected:{bg:"#fde8e8",fg:"#cc0000"}}[v.status]||{bg:"#f0f0f0",fg:"#555"};
            return(
              <div key={v.id}
                style={{...S.vCard,borderLeft:sel?"4px solid #1a6fa8":"4px solid #eee",backgroundColor:sel?"#f0f7ff":"#fff"}}
                onClick={()=>toggleSelect(v.id)}>
                <div style={S.vTop}>
                  <input type="checkbox" checked={sel} onChange={()=>toggleSelect(v.id)} onClick={e=>e.stopPropagation()} style={S.checkbox}/>
                  ...
                  {atts.length>0&&(
                    <button style={{...S.attToggleBtn,backgroundColor:expanded?"#e8f4fd":"#f5f5f5"}}
                      onClick={e=>toggleExpand(v.id,e)}>
                      📎 {activeCount}/{atts.length} {expanded?"▲":"▼"}
                    </button>
                  )}
                </div>
                <div style={S.vInfo}>
                  ...
                  <span style={{color:"#888"}}>📁 {v.departmentName}</span>
                  <span style={S.vAmt}>AED {fmtAED(v.totalAmount)}</span>
                </div>

                {/* Per-attachment checkboxes */}
                {expanded&&atts.length>0&&(
                  <div style={S.attList} onClick={e=>e.stopPropagation()}>
                    <div style={S.attListTitle}>Uncheck to skip from print:</div>
                    {atts.map((a,ai)=>{
                      const skipped=isAttSkipped(v.id,a.originalUrl);
                      return(
                        <div key={ai} style={{...S.attRow,opacity:skipped?0.45:1}}>
                          <input type="checkbox" checked={!skipped} onChange={()=>toggleSkipAtt(v.id,a.originalUrl)} style={S.checkbox}/>
                          <div style={S.attThumb}>
                            {isImageUrl(a.thumbnailUrl||a.originalUrl)
                              ?<img src={a.thumbnailUrl||a.originalUrl} alt={a.fileName} style={S.thumbImg}/>
                              :<div style={S.thumbPdf}>PDF</div>
                            }
                          </div>
                          <div style={S.attInfo}>
                            <div style={{...S.attName,textDecoration:skipped?"line-through":"none"}}>{a.fileName}</div>
                            <div style={S.attType}>{isImageUrl(a.originalUrl)?"Image":"PDF Document"}</div>
                          </div>
                          <span style={{fontSize:12,color:skipped?"#cc0000":"#2d6a4f",fontWeight:600}}>{skipped?"Skipped":"Include"}</span>
                        </div>
                      );
                    })}
                  </div>
      ...
      <div className="print-only">
        {printPlan.map((unit,ui)=>{
          ...
          const firstAtt=atts[0]; const extraAtts=atts.slice(1); const totalAtts=atts.length;
          return(
            <div key={ui}>
              <div className="p-a4-page p-has-att">
                <VoucherA5 voucher={voucher} logo={logo}/>
                <div className="p-solid-divider"/>
                <AttachmentPage attachment={firstAtt} label={`Attachment 1${totalAtts>1?` of ${totalAtts}`:""} · ${voucher.refNumber} · ${voucher.receiverName}`}/>
              </div>
              {extraAtts.map((att,ai)=>(
                <div key={ai} className="p-a4-page p-att-only">
                  <AttachmentPage attachment={att} label={`Attachment ${ai+2} of ${totalAtts} · ${voucher.refNumber} · ${voucher.receiverName}`}/>
                </div>
              ))}
            </div>
          );
      ...
      <style>{`
        ...
        @media print {
          ...
          .p-a4-page { width:100%; height:277mm; display:flex; flex-direction:column; page-break-after:always; box-sizing:border-box; overflow:hidden; }
          .p-has-att .voucher-a5 { flex-shrink:0; }
          .p-has-att .att-page   { flex:1; min-height:0; }
          .p-att-only .att-page  { flex:1; height:277mm; }
          .p-dashed-divider { border-top:1pt dashed #aaa; margin:3mm 0; flex-shrink:0; }
          .p-solid-divider  { border-top:1.5pt solid #555; margin:3mm 0; flex-shrink:0; }
          .voucher-a5 { width:100%; box-sizing:border-box; font-family:Arial,Helvetica,sans-serif; font-size:8pt; color:#111; border:0.5pt solid #ccc; padding:4mm 5mm; }
          .vh-row { display:flex; align-items:flex-start; justify-content:space-between; border-bottom:1pt solid #333; padding-bottom:2mm; margin-bottom:2mm; }
          ...
          .vi-grid { display:grid; grid-template-columns:1fr 1fr; gap:1.5mm 4mm; margin-bottom:2mm; }
          ...
          .vt-title { font-size:7pt; font-weight:bold; text-transform:uppercase; letter-spacing:0.04em; color:#555; border-top:0.5pt solid #bbb; padding-top:1.5mm; margin-bottom:1mm; }
          .vt { width:100%; border-collapse:collapse; margin-bottom:2mm; }
          .vt-h { background:#f2f2f2; border:0.5pt solid #bbb; padding:1.5mm 2mm; font-size:7pt; font-weight:bold; text-align:left; }
          .vt-d { border:0.5pt solid #ddd; padding:1.5mm 2mm; font-size:8pt; vertical-align:top; }
          .vt-alt { background:#fafafa; }
          .vb-total { display:flex; justify-content:space-between; align-items:center; border-top:0.5pt solid #bbb; border-bottom:0.5pt solid #bbb; padding:1.5mm 0; margin-bottom:2mm; }
          ...
          .att-page { display:flex; flex-direction:column; box-sizing:border-box; border:0.5pt solid #ccc; }
          .att-header { display:flex; justify-content:space-between; align-items:center; padding:1.5mm 3mm; background:#f5f5f5; border-bottom:0.5pt solid #ddd; flex-shrink:0; }
          ...
          .att-body { flex:1; overflow:hidden; display:flex; align-items:center; justify-content:center; padding:2mm; min-height:0; }
...
const S={
  ...
  layoutHint:{backgroundColor:"#f8f9fa",border:"1px solid #eee",borderRadius:8,padding:"10px 14px",marginBottom:12,display:"flex",alignItems:"center",gap:8,flexWrap:"wrap"},
  ...
  vAmt:{fontWeight:700,color:"#cc0000",marginLeft:"auto"},
