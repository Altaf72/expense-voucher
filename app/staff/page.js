        ...
        {activeTab === "history" && (
          <div>
            ...
            {filteredVouchers.map(v => {
              ...
              return (
              <div key={v.id} style={{ ...S.vCard, borderLeft: sel ? "4px solid #1a6fa8" : "4px solid #eee", backgroundColor: sel ? "#f0f7ff" : "#fff" }}>
                <input type="checkbox" checked={sel} onChange={() => toggleSelect?.(v.id)} style={S.checkbox} readOnly />
                <span style={{ fontSize:12, fontWeight:700, color:"#1a6fa8", minWidth:140, whiteSpace:"nowrap" }}>{v.refNumber}</span>
                <span style={{ fontSize:12, fontWeight:600, color:"#333", minWidth:120, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>🏢 {v.companyName}</span>
                <span style={{ fontSize:12, color:"#555", minWidth:80, whiteSpace:"nowrap" }}>📁 {v.departmentName}</span>
                <span style={{ fontSize:12, color:"#555", flex:1, overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>👤 {v.receiverName}</span>
  <span style={{ fontSize:12, color:"#888", minWidth:50, whiteSpace:"nowrap" }}>{v.items?.[0]?.category||"—"}</span>
  <span style={{ fontSize:13, fontWeight:800, color:"#cc0000", minWidth:90, textAlign:"right", whiteSpace:"nowrap" }}>AED {fmtAED(v.totalAmount)}</span>
  <span style={{ fontSize:11, color:"#888", minWidth:120, whiteSpace:"nowrap" }}>
    {v.date}{v.createdAt?.seconds ? ` ${new Date(v.createdAt.seconds*1000).toLocaleTimeString("en-GB",{hour:"2-digit",minute:"2-digit"})}` : ""}
  </span>
  <span style={{ ...S.badge, backgroundColor:sc.bg, color:sc.fg, flexShrink:0 }}>{v.status?.toUpperCase()}</span>
  {v.attachments?.length > 0 && <span style={S.attBadge}>📎{v.attachments.length}</span>}
  <div style={S.vActions}>
    <button style={{ ...S.actBtn, color:"#1a6fa8", fontWeight:600 }} onClick={() => setViewingVoucher(v)}>👁</button>
    {v.status==="pending" && <>
      <button style={S.actBtn} onClick={() => loadForEdit(v)}>✏️</button>
      <button style={{ ...S.actBtn, color:"#856404" }} onClick={() => copyVoucher(v)}>📋</button>
      <button style={{ ...S.actBtn, color:"#cc0000" }} onClick={() => withdrawVoucher(v)}>🗑</button>
    </>}
    {v.status!=="pending" && <button style={{ ...S.actBtn, color:"#856404" }} onClick={() => copyVoucher(v)}>📋</button>}
  </div>
</div>
              );
            })}
...
const S = {
  ...
  vCard:  { borderRadius:8, padding:"8px 12px", marginBottom:6, cursor:"pointer", boxShadow:"0 1px 3px rgba(0,0,0,0.06)", display:"flex", alignItems:"center", gap:10, flexWrap:"nowrap", overflow:"hidden" },
  vTop: { display:"flex", alignItems:"center", gap:8, flex:1, flexWrap:"nowrap", overflow:"hidden" },
